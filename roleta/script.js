...
function startSpin() {
  ...
  // candidatos válidos: não esgotados e não igual ao último vencedor
  const candidates = [];
  for (let i = 0; i < n; i++) {
    const label = currentItems[i];
    const rem = remainingFor(label);
    const isSameAsLast = lastWinner && label === lastWinner;
    if (rem > 0 && !isSameAsLast) candidates.push(i);
  }
  // Se não houver candidatos, permite qualquer não esgotado
  let pool = candidates;
  if (pool.length === 0) {
    for (let i = 0; i < n; i++) {
      const label = currentItems[i];
  ...
  // Se ainda vazio (tudo esgotado), usa todos para não travar
  if (pool.length === 0) {
